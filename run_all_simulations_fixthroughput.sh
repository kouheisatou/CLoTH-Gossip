...
for j in $(seq 1.0 0.2 5.0); do
    ...
    enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=gossip/avg_pmt_amt=$avg_pmt_amt,payment_rate=$payment_rate    $dijkstra_cache_dir/method=gossip,seed=$seed,n_payments=$n_payments,avg_pmt_amt=$avg_pmt_amt  payment_rate=$payment_rate n_payments=$n_payments mpp=0 routing_method=gossip group_cap_update=        average_payment_amount=$avg_pmt_amt group_size=  group_limit_rate="
    enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=group/avg_pmt_amt=$avg_pmt_amt,payment_rate=$payment_rate     $dijkstra_cache_dir/method=group,seed=$seed,n_payments=$n_payments,avg_pmt_amt=$avg_pmt_amt   payment_rate=$payment_rate n_payments=$n_payments mpp=0 routing_method=group  group_cap_update=true    average_payment_amount=$avg_pmt_amt group_size=5 group_limit_rate=0.1"
done
