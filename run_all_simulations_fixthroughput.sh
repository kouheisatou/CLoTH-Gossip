...
throughput="1000000"
for j in $(seq 1.0 0.2 5.0); do
    payment_rate=$(python3 -c "print('{:.0f}'.format(10**($j)))")
    avg_pmt_amt=$(python3 -c "print('{:.0f}'.format($throughput / 10**($j)))")
    enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=ideal/avg_pmt_amt=$avg_pmt_amt,payment_rate=$payment_rate             $dijkstra_cache_dir/method=ideal,seed=$seed,n_payments=$n_payments,avg_pmt_amt=$avg_pmt_amt           payment_rate=$payment_rate n_payments=$n_payments mpp=0 routing_method=ideal          group_cap_update=        average_payment_amount=$avg_pmt_amt group_size=  group_limit_rate="
    enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=channel_update/avg_pmt_amt=$avg_pmt_amt,payment_rate=$payment_rate    $dijkstra_cache_dir/method=channel_update,seed=$seed,n_payments=$n_payments,avg_pmt_amt=$avg_pmt_amt  payment_rate=$payment_rate n_payments=$n_payments mpp=0 routing_method=channel_update group_cap_update=        average_payment_amount=$avg_pmt_amt group_size=  group_limit_rate="
    enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=group_routing/avg_pmt_amt=$avg_pmt_amt,payment_rate=$payment_rate     $dijkstra_cache_dir/method=group_routing,seed=$seed,n_payments=$n_payments,avg_pmt_amt=$avg_pmt_amt   payment_rate=$payment_rate n_payments=$n_payments mpp=0 routing_method=group_routing  group_cap_update=true    average_payment_amount=$avg_pmt_amt group_size=5 group_limit_rate=0.1"
#    enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=cloth_original/avg_pmt_amt=$avg_pmt_amt,payment_rate=$payment_rate    $dijkstra_cache_dir/method=cloth_original,seed=$seed,n_payments=$n_payments,avg_pmt_amt=$avg_pmt_amt  payment_rate=$payment_rate n_payments=$n_payments mpp=0 routing_method=cloth_original group_cap_update=        average_payment_amount=$avg_pmt_amt group_size=  group_limit_rate="
done
