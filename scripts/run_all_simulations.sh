...
for i in $(seq 1.0 0.5 5.0); do
    ...
    enqueue_simulation         "./run-simulation.sh $seed $output_dir/enable_group_routing=false/average_payment_amount=$avg_pmt_amt                                                                                $install_dir/dijkstra_cache_$avg_pmt_amt     payment_rate=100 n_payments=50000 enable_group_routing=false                          average_payment_amount=$avg_pmt_amt"
