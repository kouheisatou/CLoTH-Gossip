...
for i in $(seq 1.0 0.5 5.0); do
    ...
    enqueue_simulation         "./run-simulation.sh $seed $output_dir/enable_group_routing=false/average_payment_amount=$avg_pmt_amt                                                                                $install_dir/dijkstra_cache_$avg_pmt_amt     payment_rate=100 n_payments=50000 mpp=0 enable_group_routing=false group_cap_update=false   average_payment_amount=$avg_pmt_amt group_size=-1 group_limit_rate=-1"

    for ((j = 2; j <= 10; j++)); do
        group_size="$j"
        for k in $(seq -3.0 0.5 0.0); do
            group_limit_rate=$(python3 -c "print('{:.4f}'.format(10**$k))")
            enqueue_simulation "./run-simulation.sh $seed $output_dir/enable_group_routing=true,group_update=true/average_payment_amount=$avg_pmt_amt/group_size=$group_size/group_limit_rate=$group_limit_rate     $install_dir/dijkstra_cache_$avg_pmt_amt     payment_rate=100 n_payments=50000 mpp=0 enable_group_routing=true group_cap_update=true     average_payment_amount=$avg_pmt_amt group_size=$group_size group_limit_rate=$group_limit_rate"
            enqueue_simulation "./run-simulation.sh $seed $output_dir/enable_group_routing=true,group_update=false/average_payment_amount=$avg_pmt_amt/group_size=$group_size/group_limit_rate=$group_limit_rate    $install_dir/dijkstra_cache_$avg_pmt_amt     payment_rate=100 n_payments=50000 mpp=0 enable_group_routing=true group_cap_update=false    average_payment_amount=$avg_pmt_amt group_size=$group_size group_limit_rate=$group_limit_rate"
        done
    done
