...
for i in $(seq 1.0 0.5 5.0); do
    ...
    for ((j = 2; j <= 10; j++)); do
        ...
        for k in $(seq -3.0 0.5 0.0); do
            ...
            enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_type=group_routing,group_update=true/average_payment_amount=$avg_pmt_amt/group_size=$group_size/group_limit_rate=$group_limit_rate     $install_dir/dijkstra_cache_$avg_pmt_amt     payment_rate=100 n_payments=50000 mpp=0 routing_type=group_routing group_cap_update=true     average_payment_amount=$avg_pmt_amt group_size=$group_size group_limit_rate=$group_limit_rate"
            enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_type=group_routing,group_update=false/average_payment_amount=$avg_pmt_amt/group_size=$group_size/group_limit_rate=$group_limit_rate    $install_dir/dijkstra_cache_$avg_pmt_amt     payment_rate=100 n_payments=50000 mpp=0 routing_type=group_routing group_cap_update=false    average_payment_amount=$avg_pmt_amt group_size=$group_size group_limit_rate=$group_limit_rate"
        done
    done
