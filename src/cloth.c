...
void write_output(struct network *network, struct array *payments, char output_dir_name[], struct array *channel_updates) {
    ...
    fprintf(describegraph_json, "{\"nodes\":[");
    for (i = 0; i < array_len(network->nodes); i++) {
        struct node *n = array_get(network->nodes, i);
        fprintf(describegraph_json, "{\"pub_key\": \"%ld\"}", n->id);
        if (i != array_len(network->nodes) - 1) {
            fprintf(describegraph_json, ",");
        }
    }
    fprintf(describegraph_json, "],\"edges\": [");
    ...
    fprintf(csv_channel_update_log,
    ...
    fprintf(csv_channel_announcement_log,
            "node_sig_1,node_sig2,bitcoin_sig_1,bitcoin_sig_2,features,chain_hash,channel_id,node_id_1,node_id_2,bitcoin_key_1,bitcoin_key_2\n");
    for (i = 0; i < array_len(network->channels); i++) {
    ...
    for (i = 0; i < array_len(network->channels); i++) {
        ...
        fprintf(csv_channel_output, "%ld,%ld,%ld,%ld,%ld,%ld,%d\n", channel->id, channel->edge1, channel->edge2,
                channel->node1, channel->node2, channel->capacity, channel->is_closed);
    }
    ...
    fprintf(csv_edge_output,
            "id,channel_id,counter_edge_id,from_node_id,to_node_id,balance,fee_base,fee_proportional,min_htlc,timelock,is_closed,tot_flows\n");
    for (i = 0; i < array_len(network->edges); i++) {
        edge = array_get(network->edges, i);
        fprintf(csv_edge_output, "%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%d,%d,%ld\n", edge->id, edge->channel_id,
                edge->counter_edge_id, edge->from_node_id, edge->to_node_id, edge->balance, edge->policy.fee_base,
                edge->policy.fee_proportional, edge->policy.min_htlc, edge->policy.timelock, edge->is_closed,
                edge->tot_flows);
    }
    ...
    fprintf(csv_payment_output,
            "id,sender_id,receiver_id,amount,start_time,end_time,mpp,is_success,no_balance_count,offline_node_count,timeout_exp,attempts,route,total_fee\n");
    for (i = 0; i < array_len(payments); i++) {
        payment = array_get(payments, i);
        if (payment->id == -1) continue;
        fprintf(csv_payment_output, "%ld,%ld,%ld,%ld,%ld,%ld,%u,%u,%d,%d,%u,%d,", payment->id, payment->sender,
                payment->receiver, payment->amount, payment->start_time, payment->end_time, payment->is_shard,
                payment->is_success, payment->no_balance_count, payment->offline_node_count, payment->is_timeout,
                payment->attempts);
        route = payment->route;
        if (route == NULL)
...
void read_input(struct network_params *net_params, struct payments_params *pay_params) {
    ...
    while (fgets(line, 1024, input_file)) {
        ...
        if (value[0] == ' ' || parameter[strlen(parameter) - 1] == ' ') {
            fprintf(stderr,
                    "ERROR: no space allowed after/before <=> character in <cloth_input.txt>. Space detected in parameter <%s>\n",
                    parameter);
            fclose(input_file);
            exit(-1);
        }
        ...
        if (strcmp(parameter, "generate_network_from_file") == 0) {
            if (strcmp(value, "true") == 0)
                ...
                fprintf(stderr,
                        "ERROR: wrong value of parameter <%s> in <cloth_input.txt>. Possible values are <true> or <false>\n",
                        parameter);
                fclose(input_file);
                exit(-1);
            }
...
void post_process_payment_stats(struct array *payments) {
    ...
    for (i = 0; i < array_len(payments); i++) {
        ...
        if (shard1->route != NULL && shard2->route != NULL) {
            payment->route = array_len(shard1->route->route_hops) > array_len(shard2->route->route_hops) ? shard1->route
                                                                                                         : shard2->route;
