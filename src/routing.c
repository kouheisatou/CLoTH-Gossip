...
void *dijkstra_thread(void *arg) {
    ...
    while (1) {
        ...
        hops = dijkstra(payment->sender, payment->receiver, payment->amount, thread_args->network,
                        thread_args->current_time, thread_args->data_index, &error);
...
struct array *get_best_edges(long to_node_id, uint64_t amount, long source_node_id, struct network *network) {
    ...
    for (i = 0; i < array_len(to_node->open_edges); i++) {
        ...
        if (!local_node) {
            ...
            new_best_edge = new_edge(best_edge->id, best_edge->channel_id, best_edge->counter_edge_id,
                                     best_edge->from_node_id, best_edge->to_node_id, best_edge->balance,
                                     modified_policy);
        } else {
...
struct array *
dijkstra(long source, long target, uint64_t amount, struct network *network, uint64_t current_time, long p,
         enum pathfind_error *error) {
    ...
    while (heap_len(distance_heap[p]) != 0) {
        ...
        for (j = 0; j < array_len(best_node->open_edges); j++) {
            ...
            distance_heap[p] = heap_insert_or_update(distance_heap[p], &distance[p][from_node_id], compare_distance,
                                                     is_key_equal);
        }
    }
