...
for i in $(seq 1.0 0.2 5.0); do
    ...
    enqueue_simulation         "./run-simulation.sh $seed $output_dir/routing_method=gossip/average_payment_amount=$avg_pmt_amt                                                                                $dijkstra_cache_dir/method=gossip,avg_pmt_amt=$avg_pmt_amt    payment_timeout=-1 payment_rate=100 n_payments=50000 mpp=0 routing_method=gossip group_cap_update=        average_payment_amount=$avg_pmt_amt group_size= group_limit_rate="

    for ((j = 2; j <= 30; j++)); do
        group_size="$j"
        for k in $(seq -3.0 0.5 0.0); do
            group_limit_rate=$(python3 -c "print('{:.4f}'.format(10**$k))")
            enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=group,group_update=true/average_payment_amount=$avg_pmt_amt/group_size=$group_size/group_limit_rate=$group_limit_rate     $dijkstra_cache_dir/method=group,avg_pmt_amt=$avg_pmt_amt     payment_timeout=-1 payment_rate=100 n_payments=50000 mpp=0 routing_method=group group_cap_update=true     average_payment_amount=$avg_pmt_amt group_size=$group_size group_limit_rate=$group_limit_rate"
            enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=group,group_update=false/average_payment_amount=$avg_pmt_amt/group_size=$group_size/group_limit_rate=$group_limit_rate    $dijkstra_cache_dir/method=group,avg_pmt_amt=$avg_pmt_amt     payment_timeout=-1 payment_rate=100 n_payments=50000 mpp=0 routing_method=group group_cap_update=false    average_payment_amount=$avg_pmt_amt group_size=$group_size group_limit_rate=$group_limit_rate"
        done
    done
