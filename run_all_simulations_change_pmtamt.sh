...
for i in $(seq 1.0 0.2 5.0); do
    ...
    for j in $(seq 0.0 0.2 1.0); do
          enqueue_simulation         "./run-simulation.sh $seed $output_dir/routing_method=group_routing/enable_fake_balance_update=true/group_limit_rate=$j/average_payment_amount=$avg_pmt_amt    payment_timeout=-1 n_payments=5000 mpp=0 routing_method=group_routing  group_cap_update=true    average_payment_amount=$avg_pmt_amt  variance_payment_amount=$var_pmt_amt  group_size=10 group_limit_rate=$j enable_fake_balance_update=true"
          enqueue_simulation         "./run-simulation.sh $seed $output_dir/routing_method=group_routing/enable_fake_balance_update=false/group_limit_rate=$j/average_payment_amount=$avg_pmt_amt   payment_timeout=-1 n_payments=5000 mpp=0 routing_method=group_routing  group_cap_update=true    average_payment_amount=$avg_pmt_amt  variance_payment_amount=$var_pmt_amt  group_size=10 group_limit_rate=$j enable_fake_balance_update=false"
    done
done
