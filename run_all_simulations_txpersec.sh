...
for ((i = 2; i <= 4; i++)); do
    ...
    for j in $(seq 0.0 0.2 3.6); do
      ...
      enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=ideal/average_payment_amount=$avg_pmt_amt/payment_rate=$payment_rate             $dijkstra_cache_dir/method=ideal,avg_pmt_amt=$avg_pmt_amt            payment_rate=$payment_rate n_payments=50000 mpp=0 routing_method=ideal          group_cap_update=        average_payment_amount=$avg_pmt_amt group_size=  group_limit_rate="
      enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=channel_update/average_payment_amount=$avg_pmt_amt/payment_rate=$payment_rate    $dijkstra_cache_dir/method=channel_update,avg_pmt_amt=$avg_pmt_amt   payment_rate=$payment_rate n_payments=50000 mpp=0 routing_method=channel_update group_cap_update=        average_payment_amount=$avg_pmt_amt group_size=  group_limit_rate="
      enqueue_simulation "./run-simulation.sh $seed $output_dir/routing_method=group_routing/average_payment_amount=$avg_pmt_amt/payment_rate=$payment_rate     $dijkstra_cache_dir/method=group_routing,avg_pmt_amt=$avg_pmt_amt    payment_rate=$payment_rate n_payments=50000 mpp=0 routing_method=group_routing  group_cap_update=true    average_payment_amount=$avg_pmt_amt group_size=5 group_limit_rate=0.1"
    done
